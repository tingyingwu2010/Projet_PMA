function newbin_process(numitem,weight,new_index,cond)
    ...
    for (i,j,k) in cond
        ...
        if (i in new_index) && (j in new_index)
            ...
            if ((k == 1) && !(i in index) & !(j in index))
                nb_larger_item = nb_larger_item + 1
    ...
    for i in new_index
        ...
        if (!Possible)
            ...
            weight_bin[nb_larger_item] = weight[i]
            push!(bin_item[nb_larger_item],i)
        end
    end
    ...
    bin_item = bin_item[1:nb_larger_item]
    weight_bin = weight_bin[1:nb_larger_item]
    ...
    return nb_larger_item,bin_item,weight_bin
end
